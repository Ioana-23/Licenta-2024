...
def read_boxes(
    boxes_fp: pd._typing.FilePath, filepaths_fp: pd._typing.FilePath
) -> pd.DataFrame:
...
def evaluate(
    labels_fp: pd._typing.FilePath,
    boxes_fp: pd._typing.FilePath,
    predictions_fp: pd._typing.FilePath,
) -> Dict[str, float]:
